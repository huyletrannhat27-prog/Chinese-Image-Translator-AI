# Xác định độ chính xác OCR + dịch thuật

> Nhiệm vụ: tích hợp công cụ để xác định lại độ chính xác của kết quả OCR
> và bản dịch trả về. UI dùng Tesseract.js để phản hồi nhanh; PaddleOCR vẫn
> có sẵn cho OCR self-host/đối chiếu. Dịch thuật dùng Gemini API.
...
Route `/api/verify` tách rõ các bước:

1. **OCR**: UI gửi kết quả Tesseract.js để phản hồi nhanh. Khi caller chỉ
   gửi ảnh, route chạy `scripts/paddle_ocr.py` qua `src/lib/ocr/paddle.ts`.
...
Route `/api/translate` xử lý trường hợp chỉ cần dịch text, có rate limit,
cache, retry và timeout.

## 2. Xác định độ chính xác OCR

Tesseract.js và PaddleOCR đều tính confidence cho từng vùng chữ - không cần
gọi thêm công cụ ngoài cho bước này. `src/lib/verification/accuracy.ts` hàm
`evaluateOcrAccuracy()`:

- Chuẩn hóa vùng OCR thành `wordBoxes[]` rồi tổng hợp `averageConfidence`.
...
- UI đã gọi `/api/verify` và hiển thị riêng độ tin cậy OCR/độ tương đồng dịch.
- Chuỗi một ký tự dùng Dice theo unigram để tránh false-negative khi không
  thể tạo bigram; chuỗi dài vẫn dùng bigram ký tự.
- PaddleOCR được giữ làm API self-host; UI ưu tiên Tesseract.js để không chịu
  thời gian khởi tạo model Python ở mỗi cold start.
- Có thể mở rộng bằng bộ dữ liệu có bản dịch chuẩn để tính BLEU/chrF/COMET;
  dịch vòng hiện chỉ là phép ước lượng khi chưa có reference.
